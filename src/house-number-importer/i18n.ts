/**
 * i18n for the house-number importer. Strings live in `locales/<lang>/common.json` under
 * the `hnImport` key.
 *
 * Like the street-name checker, this owns a DEDICATED i18next instance rather than the
 * host singleton: its language is a per-feature preference, so changing it here must not
 * flip the host's dialogs (and vice-versa).
 */
...
/** Keys of the `hnImport` namespace section, derived from the English source. */
export type StringKey = keyof (typeof enCommon)["hnImport"];
...
export function t(key: StringKey, params?: Record<string, string | number>): string {
  ...
  return i18n.t(`common:hnImport.${key}`, params) as string;
}
